    ...
    function initViewer() {{
        ...
        Autodesk.Viewing.Initializer(options, function() {{
            try {{
                ...
                log('Loading Model with fileExt: gltf...');
                // We add fileExt: 'gltf' to the second argument (options)
                viewer.loadModel(url, {{ fileExt: 'gltf' }}, function(model) {{
                    log('SUCCESS: Model loaded!');
                    setTimeout(function() {{ logDiv.style.opacity = '0.3'; }}, 5000);
                }}, function(errorCode, errorMsg) {{
                    log('LOAD ERROR: ' + errorCode + ' - ' + errorMsg);
                    if (errorCode == 13) {{
                         log('Suggestion: The viewer needs a hint that the Blob is a glTF file.');
                    }}
                }});
...
# Specifically target the cell
found = False
for i, cell in enumerate(nb.cells):
    if cell.cell_type == 'code' and ('Diagnostic Log' in cell.source or 'viewer-container' in cell.source):
