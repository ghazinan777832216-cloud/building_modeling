...
if generated_models and generated_models[0]:
    ...
    # Recenter to origin (0,0,0) to prevent precision issues
    ...
    print(f"\n✅ Center Lat/Lon offset: {center_point}")
    print(f"✅ Model centered successfully.")
else:
    print("❌ Model generation failed.")"""

v10_cell_4 = r"""from IPython.display import HTML, display
import json
...
html_code = f\"\"\"
<div id='viewer-container-v10' style='position:relative; width:800px; height:600px; background: #000; border-radius: 8px; overflow: hidden;'>
    <div id='viewer' style='width:100%; height:100%'></div>
    <div id='log-v10' style='position:absolute; bottom:0; left:0; width:100%; background:rgba(0,0,0,0.9); color:#0f0; font-family:monospace; font-size:11px; padding:10px; z-index:1002;'>
        Diagnostic Viewer v10 (FORCED ERROR 14 FIX)...
    </div>
</div>
...
(function() {{
    async function run() {{
        ...
        Autodesk.Viewing.Initializer({{ env: 'Local', useADP: false }}, async function() {{
            ...
            vLog('Viewer started. FORCING glTF extension load...');
            
            try {{
                // Wait for core to be ready
                await new Promise(r => setTimeout(r, 1000));
                
                // Explicitly register the extension if it's missing
                ...
                vLog('Loading centered glTF model...');
                viewer.loadModel(url, loadOptions, function() {{
                    vLog('SUCCESS: Building visible!');
                    viewer.fitToView();
                }}, function(c, m) {{
                    vLog('LOAD ERROR ' + c + ': ' + m);
                }});
    ...
    try:
        ...
        for cell in nb.cells:
            if cell.cell_type == 'code':
                # Identify cells by their identifiers or key content
                if 'pyprt.ModelGenerator' in cell.source:
                    cell.source = v10_cell_3
                    cell.outputs = []
                    cell.execution_count = None
                if 'Diagnostic' in cell.source or 'viewer-container' in cell.source or 'v5' in cell.source:
                    cell.source = v10_cell_4
                    cell.outputs = []
                    cell.execution_count = None
                    
        with open(p, 'w', encoding='utf-8') as f:
            nbformat.write(nb, f)
        print(f"Updated {p} to v10")
